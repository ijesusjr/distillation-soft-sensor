    ...
    try:
        ...
        if isinstance(prediction, np.ndarray):
            if prediction.ndim > 0:
                purity = prediction.flat[0]  # Get first element safely
            else:
                purity = prediction.item()  # Get scalar from 0-d array
        else:
            purity = prediction  # Already a scalar
        
        # Convert to float and clip to valid range
        purity = float(purity)
        purity = np.clip(purity, 0.0, 1.0)
        purity = float(purity)  # Ensure it's a float, not numpy.float64
